                       ...
                       string ksid, int index, vector<unsigned char> psk, bool new_receiver_inputs)
{
    ...
    qsender.my_port = my_port;
    qsender.other_player_port = other_player_port;
    qsender.counter = 0;
    qsender.mem = NULL;
    ...
    qreceiver.my_port = my_port;
    qreceiver.other_player_port = other_player_port;
    qreceiver.counter = 0;
    qreceiver.mem = NULL;
    ...
    unsigned int sender_out[2][KEY_LENGTH / 64]; // stores sender outputs
    unsigned int receiver_out[KEY_LENGTH / 64];  // stores receiver outputs
    ...
    for (i = 0; i < nOT; i++)
    {
        ...
        if ((my_num % 2 == 0 && other_player > my_num) || (my_num % 2 == 0 && other_player % 2 != 0))
        {
            if (ot_role & RECEIVER)
            {
        ...
        if (ot_role & SENDER)
        {
            for (j = 0; j < 8 * 8; j++) // generate random chars
            {
            ...
            for (j = 0; j < 8; j++) // store the generated chars in long long int format
            {
        ...
        if (ot_role & RECEIVER)
        {
            ...
            for (j = 0; j < 8; j++) // store the random chars in long long int format
            {

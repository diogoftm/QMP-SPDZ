...
void receiver_okd (OKDOT_RECEIVER * r)
{
	...
	if ((receiverfile = fopen("quantum_random_oblivious_transfer/ObliviousKeys.sgn","r")))
	{
		while (i<KEY_LENGTH)
		{
			if (fscanf(receiverfile, "%c", &r->receiver_OTkey[i]))
				i++;
			else
	...
	if ((receiverfile = fopen("quantum_random_oblivious_transfer/key_aux.txt","r")))
	{
		while (i<KEY_LENGTH)
		{
			if (fscanf(receiverfile,"%c", &r->receiver_OTauxkey[i]))
				i++;
			else
				printf ("QOT ERROR: failed to read oblivious keys.\n");
		}
